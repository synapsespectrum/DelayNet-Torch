...
class TimeSeriesGenerator:
    ...
    def __init__(
        ...
        if isinstance(config["prediction_feature"], list):
            feature_indices = [config["features"].index(f) for f in config["prediction_feature"]]
        else:
            feature_indices = config["features"].index(config["prediction_feature"])

        self.data_train = self.build_tsd(self.X_train, feature_indices)
        # (1508, X) -> [(1339, 168, X), (1339, prediction_step)]
        self.data_valid = self.build_tsd(self.X_valid, feature_indices)
        if self.X_test is not None:
            self.data_test = self.build_tsd(self.X_test, feature_indices)
        else:
            self.data_test = None
...
class Dataset:
    ...
    def __init__(self, dataset_name, noise_std=0.0, smoothing_window=0):
    ...
    def __load_data(self):
        if self.dataset_name == cnu_str or \
            ...
            return FRANCEHOUSEHOLD(noise_std=self.noise_std, smoothing_window=self.smoothing_window)
            ...
            return GYEONGGI(data_name=self.dataset_name, noise_std=self.noise_std, smoothing_window=self.smoothing_window)

        elif self.dataset_name == spain_str:
            return SPAIN(noise_std=self.noise_std, smoothing_window=self.smoothing_window)

...
from delayedtcn.baselines import GRUModel, TCNModel

# ... (existing code)

def get_model(config) -> object:
    model_name = config.get('model_name', 'DelayNet')
    if model_name == 'GRU':
        return initialize_baseline_model(config, GRUModel)
    
    if model_name == 'TCN':
        return initialize_baseline_model(config, TCNModel)
    
    return initialize_model1(config)


def build_callbacks(tensorboard_log_dir='logs', tensorboard_name=None, filepath='best_weights.h5'):
...
def compile_model(model, config):
    ...
    input_test1 = Input(shape=(config.get('original_input_width', 168), len(config['features'])))
